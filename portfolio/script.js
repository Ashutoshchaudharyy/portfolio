...
const CONTACT_CONFIG = {
  ...
  formSubmit: {
    email: "your-email@example.com" // CHANGE THIS to your email to receive form submissions via FormSubmit.co
  }
};
...
const certificatePreviews = {
  ...
  "Artificial Intelligence Fundamentals":
    "/image/certificate-ai-oracle.png",
...
// ===== REUSABLE HOVER PREVIEW CLASS COMPONENT =====
class HoverPreview {
  constructor(options) {
    this.cards = document.querySelectorAll(options.cardsSelector);
    this.previewElement = document.getElementById(options.previewId);
    this.previewImg = document.getElementById(options.imgId);
    this.previewSpinner = document.getElementById(options.spinnerId);
    this.previewTitle = document.getElementById(options.titleId);
    this.previewsMap = options.previewsMap;
  ...
  init() {
    if (!this.cards.length || !this.previewElement) return;

    this.cards.forEach((card) => {
      card.addEventListener("mouseenter", (e) => this.handleMouseEnter(card, e));
  ...
  updatePosition(e) {
    ...
    const previewWidth = this.previewElement.offsetWidth || 272;
    const previewHeight = this.previewElement.offsetHeight || 262;
  ...
  handleMouseEnter(card, e) {
    ...
    this.previewImg.classList.remove("loaded");
    if (this.previewSpinner) this.previewSpinner.style.display = "block";

    this.previewImg.style.display = "block";

    if (e) this.updatePosition(e);

    const rawPath = this.previewsMap[title];
    if (!rawPath) {
      const fallbackUrl = `https://placehold.co/600x450/0a0a1a/00ffdc?text=${encodeURIComponent(title)}`;
      this.previewImg.src = fallbackUrl;
      this.previewImg.classList.add("loaded");
      if (this.previewSpinner) this.previewSpinner.style.display = "none";
    ...
    tempImg.onload = () => {
      if (this.lastCard === card) {
        this.previewImg.src = resolvedPath;
        this.previewImg.classList.add("loaded");
        if (this.previewSpinner) this.previewSpinner.style.display = "none";
      }
    };
    ...
    tempImg.onerror = () => {
      if (this.lastCard === card) {
        this.previewImg.src = `https://placehold.co/600x450/0a0a1a/00ffdc?text=${encodeURIComponent(title)}`;
        this.previewImg.classList.add("loaded");
        if (this.previewSpinner) this.previewSpinner.style.display = "none";
      }
    };
  ...
  handleMouseLeave() {
    ...
    this.cleanupTimeout = setTimeout(() => {
      if (!this.previewElement.classList.contains("visible")) {
        this.previewImg.src = "";
        this.previewImg.classList.remove("loaded");
      }
    }, 450);
...
// ===== INITIALIZE HOVER PREVIEW COMPONENT =====
function initHoverPreview() {
  new HoverPreview({
    cardsSelector: ".cert-card",
    previewId: "certHoverPreview",
    imgId: "certHoverImg",
    spinnerId: "certHoverSpinner",
    titleId: "certHoverTitle",
    previewsMap: certificatePreviews
  });
}
